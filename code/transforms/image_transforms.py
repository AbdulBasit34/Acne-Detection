...
class Gamma(object):
    def __init__(self, value):
        ...
        Out = In ** gamma after scaling to [0, 1].
        """
        self.value = value
    ...
    def _apply_gamma(self, img):
        ...
        img = img.float()  # Ensure float dtype
        return th.pow(img, self.value)
...
class Contrast(object):
    ...
    def __call__(self, *inputs):
        outputs = []
        for idx, _input in enumerate(inputs):
            # Ensure the input has 3 dimensions (C, H, W)
            if _input.dim() == 2:  # Grayscale (H, W)
                _input = _input.unsqueeze(0)  # Convert to (1, H, W)
            elif _input.dim() == 3 and _input.shape[0] not in [1, 3]:
                _input = _input.permute(2, 0, 1)  # Convert (H, W, C) to (C, H, W)
