...
def safe_divide(numerator, denominator):
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        result[~np.isfinite(result)] = 0  # replace inf and NaN with 0
    return result
...
def sensitivity_specificity_support(y_true, y_pred, labels=None, pos_label=1, average=None, warn_for=('sensitivity', 'specificity'), sample_weight=None):
    ...
    tp = y_true == y_pred
    tp_sum = np.bincount(y_true[tp], minlength=len(labels))
    pred_sum = np.bincount(y_pred, minlength=len(labels))
    true_sum = np.bincount(y_true, minlength=len(labels))
    tn_sum = y_true.size - (pred_sum + true_sum - tp_sum)

    sensitivity = safe_divide(tp_sum, true_sum)
    specificity = safe_divide(tn_sum, tn_sum + pred_sum - tp_sum)

    if average == 'micro':
        sensitivity = safe_divide(tp_sum.sum(), true_sum.sum())
        specificity = safe_divide(tn_sum.sum(), (tn_sum + pred_sum - tp_sum).sum())

    return sensitivity, specificity, true_sum


def sensitivity_score(y_true, y_pred, labels=None, pos_label=1, average='binary', sample_weight=None):
    s, _, _ = sensitivity_specificity_support(y_true, y_pred, labels=labels, pos_label=pos_label, average=average, warn_for=('sensitivity', ), sample_weight=sample_weight)
    return s


def specificity_score(y_true, y_pred, labels=None, pos_label=1, average='binary', sample_weight=None):
    _, s, _ = sensitivity_specificity_support(y_true, y_pred, labels=labels, pos_label=pos_label, average=average, warn_for=('specificity', ), sample_weight=sample_weight)
    return s
...
def make_index_balanced_accuracy(alpha=0.1, squared=True):
    def decorate(scoring_func):
        ...
        def compute_score(*args, **kwargs):
            ...
            if squared:
                _score = np.power(_score, 2)
            sen, spe, _ = sensitivity_specificity_support(*args, **kwargs)
            dom = sen - spe
            return (1. + alpha * dom) * _score
        return compute_score
    return decorate
